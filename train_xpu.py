...
def get_gpu_mem():    
        ...
        if hasattr(torch, 'xpu') and torch.xpu.is_available():            
            try:
                ...
                result = subprocess.check_output(
                    "xpu-smi dump -m 0 -d 0 | tail -1 | awk -F',' '{print $3}'", 
                    shell=True
                ).strip().decode()
                if result:
                    mem_mb = float(result)
            ...
            print("Warning: Could not detect XPU memory, defaulting to 16GB")
            return 16
                    
        try:
            import subprocess
            result = subprocess.check_output(
                'nvidia-smi -i 0 --query-gpu=memory.total --format=csv,noheader,nounits',
                shell=True
            ).strip().decode()
    ...
    print("Warning: No GPU detected, defaulting to 16GB")
    return 16
...
if hasattr(torch, 'xpu') and torch.xpu.is_available():
    ...
    if hasattr(torch.backends, 'xpu'):
        if hasattr(torch.backends.xpu, 'allow_tf32'):
            torch.backends.xpu.allow_tf32 = True
...
class SequenceLightningModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        if not self.hparams.train.disable_dataset:
            if DEVICE == "xpu" and self.hparams.dataset.get('preload_to_xpu', False):
                print("Enabling XPU dataset preload...")
                # 修改 dataset 實例以啟用預載
                if hasattr(self.dataset, 'dataset_train'):
                    self.dataset.dataset_train.preload_to_xpu = True
                    self.dataset.dataset_train._preload_to_xpu()
            self.dataset.setup()
        ...
        if DEVICE == "xpu":
            ...
            self.encoder = self.encoder.to("xpu")
            self.decoder = self.decoder.to("xpu")
            if hasattr(self.task, 'to'):
                self.task = self.task.to("xpu")
            if hasattr(self.loss, 'to'):
                self.loss = self.loss.to("xpu")
            if hasattr(self.loss_val, 'to'):
                self.loss_val = self.loss_val.to("xpu")
    ...
    def train_dataloader(self):
        loader_args = self.hparams.loader.copy()   

        if self.hparams.dataset.get('preload_to_xpu', False):
            loader_args['num_workers'] = 0
            loader_args['pin_memory'] = False
            loader_args['persistent_workers'] = False
        
        return self.dataset.train_dataloader(**loader_args)    
...
def create_trainer(config, **kwargs):
    ...
    if n_devices > 1:
        if is_xpu_available:
            ...
            if config.trainer.get('strategy', None) is None:
                print("Configuring XPU multi-device training with XCCL backend")
                config.trainer.strategy = dict(
                    _target_='pytorch_lightning.strategies.DDPStrategy',
                    process_group_backend='xccl',  # Intel XPU 使用 XCCL 而非 NCCL
    ...
    if config.callbacks.get("seqlen_warmup_reload", None) is not None:
        ...
        trainer_config_dict.pop('_target_')        
...
def train(config):
    ...
    is_xpu_available = hasattr(torch, 'xpu') and torch.xpu.is_available()
    if is_xpu_available:
        print(f"Training on XPU: {torch.xpu.get_device_name(0)}")
        print(f"XPU device count: {torch.xpu.device_count()}")

        torch.xpu.set_device(0)
    else:
        print("Training on CPU (XPU not available)")
